...
def get_final_total_text(page):
    return wait_for_element_with_data_test_id(page,'pd-value-TOTAL').text_content()

def get_final_accommodation_text(page):
    return wait_for_element_with_data_test_id(page,'pd-value-ACCOMMODATION').text_content()

def get_final_fee_text(page):
    for fee_type in ["pd-value-CLEANING_FEE", "pd-value-AIRBNB_GUEST_FEE"]:
